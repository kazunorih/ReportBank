...
function message(error: unknown) {
  ...
  if (error.name === "UsernameExistsException") return "このメールアドレスは登録済みです。";
  if (error.name === "NotAuthorizedException") return "メールアドレスまたはパスワードが違います。";
  if (error.name === "CodeMismatchException") return "確認コードが違います。";
