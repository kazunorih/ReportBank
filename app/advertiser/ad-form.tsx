...
export default function AdForm() {
  ...
  return <form action={action} className="grid gap-6">
    ...
    <label className="grid gap-2 font-medium">本文<textarea name="body" maxLength={4000} required rows={16} onChange={(event) => setLength(event.target.value.length)} className="rounded-xl border border-slate-300 px-4 py-3" /><span className="text-right text-sm text-slate-500">{length.toLocaleString("ja-JP")} / 4,000字</span></label>
